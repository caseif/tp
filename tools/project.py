...
def generate_build_ninja(
    ...
    mwcc_pch_extab_cmd = f"{CHAIN}{mwcc_pch_cmd} && {dtk} extab clean --padding \"$extab_padding\" $out $out"
    mwcc_pch_extab_implicit: List[Optional[Path]] = [*mwcc_pch_implicit, dtk]
    ...
    if os.name != "nt":
        ...
        mwcc_pch_extab_cmd += f" && $python {transform_dep} $basefile.d $basefile.d"
        ...
        mwcc_pch_extab_implicit.append(transform_dep)
    ...
    write_custom_step("pre-compile", extra_inputs=[get_pch_out_name(config, pch) for pch in config.precompiled_headers])
    ...
    if build_config:
        ...
        if config.precompiled_headers:
            for pch in config.precompiled_headers:
                pch_out_name = Path(pch["source"]).with_suffix(".mch")
                ...
                n.build(
                    ...
                    variables={
                        ...
                        "cflags": make_flags_str(pch["cflags"]),
